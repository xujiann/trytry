...
KNOWN_ORPHANS: set[str] = {
    ...
    #
    # 2026-09-18 补：选项已写成 docs/adr/0024-secondments两套端点的收口.md（Proposed）。
    # 取证时实测到一件与"要不要接界面"直接相关的事：这条端点的 end_date 是**未校验的
    # 裸 str 查询参数**，`?end_date=完全不是日期` 200 落库，该派驻随后被 dispatch-stats
    # 整条跳过（真派满半年的中级医师从国家监测指标里消失）。今天这条路径只能经 API
    # 直连触及——正因为它没有界面。**所以在 ADR 第一步（补日期校验）落地之前别接它**：
    # 接一个带日期输入框的入口，等于把这条路径从"要直连 API"降到"页面上点两下"。
    # 裁定落地后这段旁注要跟着改写。
