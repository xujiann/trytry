...
def business_today() -> date:
    ...
    **这不是 flake，是真缺陷**：判据（期望值）与被判对象（服务端算出的日期）
    ...
    上面那段写下来的时候还有一句「窗口没有归零」——真要归零得让 `app/clock.py`
    成为全部日期的唯一来源再冻结它，而当时 `app/` 里还有 69 处各自问系统时钟。
    P1-53（2026-09-11）把那 69 处收敛掉了，于是本函数**直接走同一个入口**：

    * 不冻结时：与 `date.today()` 等价，行为同以前；
    * 冻结时（`freeze_business_date()`）：判据与被判对象取的是**同一个冻住的值**，
      窗口真正归零，用例不必再依赖"发请求与断言之间不跨午夜"。

    这一行改动的意义全在这里——如果本函数还自己去问 `date.today()`，
    冻住服务端就会让两边**对不上**，比不冻更糟。
    """
