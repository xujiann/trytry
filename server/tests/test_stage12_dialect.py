...
def test_手写SQL总量可控():
    ...
    25 → 41（2026-09-04，P1-30 逐表审计后的第二批下沉）：又新增八条**部分唯一索引**
    （专病在管、双通道待审、特病待审、长期医嘱在执行、服务包绑定中、开放咨询会话、
    待受理服务申请、待呼叫外呼任务），同样每条两处谓词，故 8 条 = 16 处。增量全部落在
    "声明"那一类，没有一处新的手写查询串——按下面这条读法，是良性的。

    **给后来者的读法**：这个数字如今由**声明**（部分索引谓词，占 40/41）而不是
    **查询**主导——真正该警惕的是新增的手写查询串（当前仍只有 routers/monitor.py
