...
#:
#: 静默截断 139 → 8，剩下的 8 个都在 `静默截断` 桶里等人看，不当豁免。
#:
#: 刻意**没动**两类：
#: ① **扫描上限**——`.limit()` 限的是扫描范围而不是输出条数，下面还要按条件筛
#:    或重排（`billing:deposit_alerts`、`appointments:find_doctors`）。改走
#:    `paginate` 会去翻"扫描的第 501~1000 条"，`X-Total-Count` 也会报成扫描池
#:    的大小而不是结果数——两个都是错的答案。两处都在代码里就地写明了，
#:    仍算在欠账里，不当豁免。
