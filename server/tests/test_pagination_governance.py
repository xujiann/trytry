...
#: **只允许调小。** 轨迹：214（本闸门建成时的实测）→ 172 → 83 → **78**。
#:
...
#: 静默截断 139 → 8 → **3**。
#:
#: 那 8 个「扫描上限」里有 5 个已经改掉了（83 → 78）：`.limit()` 之所以限的是
...
#:
#: 剩下的 3 个仍在 `静默截断` 桶里等人看，不当豁免：
#: ① `billing:deposit_alerts`——gap = 押金余额 - 未结费用，两个都是跨表聚合，
#:    下推要先把两笔聚合写成子查询（顺带还能拆掉现在的 N+1），行为与性能都要重测；
