...
#: **只允许调小。** 轨迹：214（本闸门建成时的实测）→ **172**。
#:
#: 172 这一轮做的是**纯输出上限**那一类（billing 7 / admin_mgmt 6 / inpatient 5 /
#: appointments 3 / portal 10 / spd/portal 11 = 42 个端点）：`limit` 入参的默认值
#: 取原来写死的硬上限，于是不传参的老调用方拿到的行一字不差，只多一个
#: `X-Total-Count` 响应头。
#:
#: 刻意**没动**两类：
