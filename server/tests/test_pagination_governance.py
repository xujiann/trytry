...
#: **只允许调小。** 轨迹：214（本闸门建成时的实测）→ 172 → 83 → 78 → **77**。
#:
#: 两轮做的都是**纯输出上限**那一类：`limit` 入参的默认值取原来写死的硬上限，
#: 于是不传参的老调用方拿到的行一字不差，只多一个 `X-Total-Count` 响应头。
#: 第一轮 42 个手写（billing/admin_mgmt/inpatient/appointments/portal/spd/portal，
#: 先挑居民看得见的那些）；第二轮 84 个由改写脚本做，判据收到**只认三种
#: 结构保持形态**：`return <链>`、`return [f(x) for x in <链>]`（推导式不带 if）、
#: `name = <链>` 且 `return [f(x) for x in name]`（同样不带 if）。
#: 这条收紧正是用来把「扫描上限」挡在外面的——截断之后又筛（带 if）或又排
#: （`return sorted(rows, ...)`）的，截断的不是输出而是扫描池。脚本因此自动
#: 留出 8 个待人工判断，其中两个恰好就是先前读代码时手工认出来的那两个。
#:
#: 静默截断 139 → 8 → 3 → **2**。
#:
#: 那 8 个「扫描上限」里有 6 个已经改掉了（83 → 77）：`.limit()` 之所以限的是
#: 扫描范围而不是输出条数，是因为**筛选发生在截断之后**——那本身就是个漏报缺陷
#: （P1-54），修法是把判定下推到 SQL。判定一下推，上限自然变成输出上限，
#: 于是它们也就能走 `paginate` 了。**先修缺陷、再谈分页**，顺序反过来只会把
#: 漏报固化成翻页错乱。五处的回归证据在 `tests/test_truncate_before_filter_regression.py`
#: （每条都把匹配项放在原硬上限之外，否则证明不了任何事）。
#:
#: `billing:deposit_alerts` 的两笔跨表聚合也写成子查询下推了（顺带拆掉了它的
#: N+1：改前每个在院患者两次聚合查询）。**取整必须跟着一起下推**——少一层
#: `round` 就会在恰好等于阈值的边界上与 Python 分叉（`0.3 - 0.1` 在浮点上是
#: `0.19999999999999998`），这条有用例钉着。
#:
#: 剩下的 2 个仍在 `静默截断` 桶里等人看，不当豁免：
#: ① `appointments:find_doctors`——截断之后按余号**重排**，不是筛；下推要先让
#:    排序键进 SQL，形状与前六处不同；
#: ② `portal:portal_my_contract`——上限嵌在对象里的子清单上。
#: ② **无上限**那 74 个：给它们加默认上限会**真的**截断，属行为变更，
#:    要逐条判断（按单个患者收敛的清单行数由一个人的历史决定，加上限反而漏记录；
#:    全域清单才是真危险）。单独立项。
BASELINE_UNPAGED = 77
