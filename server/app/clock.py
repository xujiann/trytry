...
- **零基线**：`date.today()` 绕过 `today()` 的处数已由 69 收敛到 **0**（P1-53，2026-09-11），
  连 `from .clock import today` 这种写法也一并禁掉——它在 import 那一刻就把函数绑成了
  本模块的名字，**逃得过冻结**。
  收敛的动机不是整齐，是**能冻得住**：`tests/conftest.py` 的 `freeze_business_date()`
  换掉本模块的 `today` 属性，就覆盖了平台全部业务日期取值；
  只冻住其中一部分比不冻更难排查——你会以为时间是固定的。
