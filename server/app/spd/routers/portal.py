...
from ...concurrency import ensure_present, insert_if_absent, insert_or_conflict, serialized_on
...
async def upload_task_evidence(
    ...
    with serialized_on(db, SpdTask, task.id):
        ...
        db.flush()
        db.refresh(task)
        if str(attachment.id) not in {str(e) for e in task.evidence or []}:
            task.evidence = [*(task.evidence or []), attachment.id]
        db.commit()
