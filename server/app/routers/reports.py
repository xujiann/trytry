...
#: 表格软件把以这些字符开头的单元格当公式算（CSV 注入）。制表符与回车（P2-116）：有的表格软件先吃掉开头的空白再求值，
#: 「\t=HYPERLINK(...)」照样是公式（OWASP 的清单里本就有这两个）。与前端 `spdDownloadCsv` 的字符类逐字一致
#: （`tests/test_csv_formula_injection.py` 对着两边比）
_FORMULA_LEAD = ("=", "+", "-", "@", "\t", "\r")
...
def _csv_cell(value):
    """以 = + - @（及制表符、回车，P2-116）开头的非数字文本前置单引号，免得被 Excel / WPS 当公式执行（P2-63）。
