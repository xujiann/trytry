...
class CourseStatsOut(BaseModel):
    ...
    trainees: int
    passed: int
    # 无人参训时是 0.0（字面量 float，不是 round 出来的）——照 handler 建模
    pass_rate_pct: float
...
class LiveRequestedOut(BaseModel):
    """申请直播回三个字段；审核/结束/挂回放各回各的，形状都不同，不硬凑。"""
...
class LiveFeedbackSubmittedOut(BaseModel):
    id: int
    # 一人一场一条，重复提交按覆盖——updated 告诉调用方这次是改不是新增
    updated: bool


class LiveFeedbackRowOut(BaseModel):
    id: int
    user_id: int
    rating: int
    comment: str
...
class LiveFeedbackListOut(BaseModel):
    session_id: int
    count: int
    # 一条反馈都没有时是 null，不是 0——"没人评价"与"均分 0 分"是两回事
    avg_rating: float | None
    feedbacks: list[LiveFeedbackRowOut]
...
@router.post("/live-sessions/{session_id}/feedback", response_model=LiveFeedbackSubmittedOut,
             status_code=201)
...
@router.post("/training-plans/{plan_id}/enroll", response_model=EnrollmentOut, status_code=201)
...
@router.get("/training-plans/{plan_id}/assessments", response_model=AssessmentListOut)
