...
class SecondmentOut(BaseModel):
    """派驻回执/台账行/结束回执共用的 `_out()` 形状（18 键，字段序=出键序）。

    `days` 为自然日 int；`end_date` 空串表示仍在派（不是 null，同列约定）。
    """
...
class DispatchOrgOut(BaseModel):
    org_id: int
    org_name: str
    ongoing: int
    total: int
...
class DispatchStatsOut(BaseModel):
    """`group_id` 是键恒在值可空（未按分组筛为 null）→ `int | None`，非条件键。"""

    year: int
    group_id: int | None
    orgs: list[DispatchOrgOut]
...
def dispatch_stats(
    ...
    return {
        ...
        # 日期非法的条数。**单列而不丢弃**——一条记录无声无息地从指标里消失，
        # 比指标少一个人更难查。
        #
        # ⚠️ 这个数**不是"非法记录的全集"**，它有一个盲区（ADR-0024 实测三）：
        # 上面那道 SQL 预筛 `end_date >= year_start_str` 是**字典序**比较，非法值若
        # 排在年初字符串之前（如 `"!!!"`、`"0000-00-00"`）会在计数之前就被排除掉，
        # 实测写进去之后这个计数器纹丝不动。所以看到 0 只说明"看得见的那一半没有"，
        # 不能据此说没丢人；要查全得直接扫表。
        #
        # 新数据这一头已经堵上：写 `secondments` 的四条端点（两条建档的 body 字段用
        # `DateStr`、两条结束的查询参数用 `deps.require_date`）都在入口挡下非法日期，
        # 所以这个数今天只该来自存量。**这句话上一次写下来时是假的**——当时只核了
        # 建档那一侧，而两条结束端点从来不校验（ADR-0024），教训是断言别人的入口
        # 之前先去看一眼那个入口。
        "invalid_date_records": invalid_date,
        "caliber": {
            "long_term_6m": f"派驻类型为长期派驻且当年在派满 {LONG_TERM_DAYS} 天的人次"
                            "（跨年派驻只计落在本年度内的天数；天数按起止日期相减计，"
                            f"故满 {LONG_TERM_DAYS} 天对应 {LONG_TERM_DAYS + 1} 个自然日）",
