...
![tests](https://img.shields.io/badge/tests-3045%20passed%20%2B%2034%20skipped-brightgreen)
...
| 手术麻醉 | 申请→审批（申请人不得自批）→手术间排班（区间重叠拦截）→术中记录；填病案首页时手术栏留空即取已完成的术式（首页建好后不回头更新） | `/api/surgery` |
...
| 门急诊文书 | 知情告知书（模板化，**开具时冻结正文**，拒签为独立状态并记原因）、门急诊治疗处置记录、门急诊护理记录、就诊文书完整性自查（只列事实不判合格） | `/api/outpatient` |
