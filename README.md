...
![tests](https://img.shields.io/badge/tests-2950%20passed%20%2B%2030%20skipped-brightgreen)
...
```bash
cd server
python -m pytest tests/ -q                                        # 全量单元/接口测试（实测 2950 项通过 + 30 项跳过；跳过项为 e2e（11 条）与真 PG 集成档等，需显式开启）
