...
- ☐ **① 慢专病配置域没有管理页面（86 个端点）** —— 缺口最大也最整齐的一块。
  量表 7 · 团队 5 · 村医档案 5 · 设备 3 · 服务包 3 · 积分规则 3 · 干预模板 2 · 标签 2 · 路径节点 3 · 宣教素材 3 …
  后端刚在 ADR-0008 那轮做完 58 个配置端点的契约治理（648 → 588，已进 `FULLY_GOVERNED`），**前端一个配置页面都没有**——
  子系统能跑，但配不了：量表配不了、团队建不了、村医档案录不进，只能拿演示数据演给人看。
  正因为契约刚做完、响应形状确定、有特征化网钉着，照着契约写页面是**返工风险最低**的一种活。**建议作为下一步第一件事。**
