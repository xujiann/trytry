...
- ✅ **P1-54 修掉 5/7：把筛选判定下推到 SQL，漏报的行回来了（欠账 83 → 78）。**
  ...
  **顺序是先修缺陷再谈分页**，反过来只会把漏报固化成翻页错乱。
  剩 2 处各有各的形状，没硬修：`billing:deposit_alerts`（gap 由两笔跨表聚合算出，
  下推要写成子查询，顺带能拆掉现在的 N+1）、`appointments:find_doctors`
  （截断之后按余号**重排**而不是筛，要先让排序键进 SQL）。
