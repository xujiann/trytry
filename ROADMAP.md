...
- ◐ `created_at` 欠账迁移：按背包清单逐张补。已补 `voucher_entries`、`fund_settlements`、`maternal_visits`、`child_visits`、`visit_credentials`、`qc_records`、`spd_measurements`（spd 链）、`vaccination_records`、`exam_reports`、`health_monitor_records`、`infectious_cases`、`prescription_items`，基线 **52→40**；下一批 `drug_stocks` / `appointment_slots` / `duty_rosters`（`blood_stocks` 为小型 upsert 表、`admissions` 属核心表需先 ADR，均降级）。
