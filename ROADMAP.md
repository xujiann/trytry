...
- ◐ **ADR-0009 第二批（2026-08-26）：又迁六页**——随访 / 定时任务 / 满意度 / 站内消息 / 质量指标 / 运行监控，`pages-mgmt.js` 里 16 处手写外壳换成 `panel()`，六页输出逐字符一致。三件事值得记：
  ...
  - 登记两条不在本轮修的（`docs/TECH_DEBT.md`）：**P2-25** 裸 `${MAP[key]}` 查不到时显示字面量 `undefined`（同族，但只是显示缺陷、非崩溃非 XSS，刻意不并进那两条守卫——一条守卫混两种严重度迟早因噪声被加豁免；且需逐处定文案）；**P2-26** 本轮修的 33 处全是在手抄 `spdTag()` 已经做对的事，收敛过去正是 ADR-0009 的论点，但不是纯 no-op（空状态码下两者显示不同），随逐页迁移一起做。
