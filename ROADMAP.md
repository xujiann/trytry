...
## 可用性线（新开 · 2026-08-25 完成度盘点的产出）

> **为什么单开一条线**：此前路线图只有治理线，等于把"完成度"定义成了"代码质量欠账还剩多少"。
> 按那个定义平台已接近完成；按"一个县能不能开机干活"的定义还差得多。
> 盘点用四层记账（功能条目 / 工程质量 / 产品可用性 / 生产就绪），
> 结论是**短板已经从「后端有没有」转移到「人能不能用上」**，而这一层此前没有棘轮、没有条目、没人在推。

盘点实测（基线 8ff5a17，全量套件 2341 passed / 20 skipped / 914s）：

- **功能条目无缺口**：36 主项 / 379 子功能已全部落地，第七轮降到子功能级、第十一轮逐条机检复核过，不重复核。
- **前端触达 812/946 ≈ 85%**，约 130 个端点在任何前端里都调不到。去掉本就是机器面的 9 个 FHIR/HL7 入站后，缺口高度集中在三处。

判据算法：两侧路径都压成"去掉参数段后的字面段签名"再比对（普通字符串与模板字符串都收，`${…}` 段按参数段丢弃）。
⚠️ **这个数有几个百分点的残余误差**——扫不到动态拼接得太厉害的调用（复核时抓到体质辨识、病历质控规则两处被误判成缺口）。
结构性结论（配置域整块缺、打印件整批缺）是稳的；**要拿它当验收指标，得先照本仓库惯例做成带守卫的棘轮用例**，不要直接引用这个数字。

- ◐ **① 慢专病配置域没有管理页面（86 → 71 个端点）** —— 缺口最大也最整齐的一块。
  后端在 ADR-0008 那轮做完 58 个配置端点的契约治理（648 → 588，已进 `FULLY_GOVERNED`），**前端一个配置页面都没有**——
  子系统能跑，但配不了：量表配不了、团队建不了、村医档案录不进，只能拿演示数据演给人看。
  正因为契约刚做完、响应形状确定、有特征化网钉着，照着契约写页面是**返工风险最低**的一种活。
  按后端子模块分批，一批一页，与当初拆包的边界对齐。
  - ✅ **第一批 `config/scales.py` 四个域（15 个）**：评估量表 7 · 宣教素材 3 · 服务包 3 · 专病标签 2。
    新页面 `spdconfig`（管理端第 90 页），量表走 draft → 配题 → publish 三段，与后端语义一一对应。
    入口棘轮 `test_spd_config_frontend.py` 逐**操作**钉住（14 个），不是"端点字符串出现过就算"——
    第一版就是那么写的，变异验证没咬住才改的：同一端点在一页里通常被引用两次，删一次字符串还在。
  - 搬出来两个真问题（都已修，带回归）：① 后端 **POST 查题目 key 重复、PATCH 不查**，
    而"新建只给壳、题目随后 PATCH 上去"正是配题的常规走法，等于查重形同虚设——
    重复 key 会让 `score_scale` 把同一个答案**计两次分**，量表照样出分、出的是错的风险等级；
    ② 前端 `spdModal` 把空的 `type:"number"` 字段转成 **0** 而不是空串，
    而评分区间的 `max: 0` 与 `max: null` 是两回事——后者是"不限"，前者让该区间永远命不中，
    "N 分以上=高危"配出来是死的。三个可空数值字段改用文本收 + `spdOptionalNumber`，守卫钉住。
  - ✅ **第二批 `config/teams.py` 三个域（12 个）**：服务团队 5 · 村医档案 5 · 团队成员 2。
    新页面 `spdteams`（管理端第 91 页），含团队成员权限矩阵、村医绑定码、批量开通逐行结果。
    入口棘轮累计 7 个域 / 26 个操作。
  - 🔴 **搬出来一个越权洞（已修，带回归 + 探针实证）**：`PATCH /api/spd/team-members/{id}` 与
    `DELETE /api/spd/team-members/{id}` **没有机构归属校验**——非全域角色（doctor）能改、能删
    **别家机构**团队的成员，而同一组的 `POST /teams/{id}/members` 是拦住的（那个有 `assert_org_writable`）。
    改成员权限直接决定那个人在移动端能看谁的患者、能不能审核转诊；删成员是把人从别人团队里踢出去。
    实测四条：加成员 403（已拦）/ 改成员 **200 放行** / 删成员 **204 放行** / 改村医档案 403（已拦）。
    修法是按所属团队解出 `org_id` 再走 `assert_org_writable`（抽 `_assert_member_org_writable`）。
    **⚠️ 这是行为变更**：此前跨机构能成功的调用现在回 403，按 CLAUDE.md §8 需人复核。
    - **为什么闸门没报**：`test_stage15_horizontal` 的 AST 扫描只认 `db.get(带 org_id 的模型, …)`，
      而 `SpdTeamMember` 的机构归属跟着所属团队走、自己没有 `org_id`——落在**盲区**里，
      不是已声明的豁免。同类形状（归属靠外键间接推导的表）值得单独扫一遍，另案。
    - **探针踩过的坑记一笔**：`director` 在 `visibility.GLOBAL_ROLES` 里、按设计全域可写，
      拿 director 探这个洞会得到「全放行」的假象，误判成"守卫整个没生效"。非全域角色用 `doctor`。
...
- ☐ **② 八张打印件没有入口（8 个端点）** —— 出院小结 · 住院费用清单 · 结算单 · 转诊单 · 接种证 · 体检报告 · 知情同意书 · 病案首页。
  后端全做完了，前端没有按钮（已有入口的是检查报告/检查申请/处方/证明/模板五类，走 `openPrintPage()`）。打印是医院日常刚需，性价比最高的一批。
- ☐ **③ 后端完整、前端零入口（12 个端点）** —— 住院押金 5（预交/退费/余额/告警，居民端只读得到余额）· CSSD 申领 3 · 双因子认证 3 · 智能导诊 1。
  其中住院押金要不要管理端录入页是**产品决策**：取决于现场是住院处用这套系统收钱、还是仍走 HIS 收费窗口。
- ☐ **④ 医生移动端补到能上岗** —— 现在只接了 16 个接口，村医随访、乡镇医师接诊、审方待办都进不去。基层医生是真正的高频用户。
- ☐ **⑤ 11 条 e2e 接进 CI** —— CI 现在显式 `-m "not integration and not e2e"`。Chromium 与 Playwright 环境本就具备，接上就是全栈链路第一次有自动验证。

### 需业务方给输入，工程侧推不动（挂着，别当排期问题）

一套国产库环境（跑一次 `alembic upgrade heads` 就知道适配清单差多少）· 医保沙箱账号 · 一家试点乡镇卫生院的 HIS 联系人 ·
《监测指标体系（2024版）》原文（驾驶舱"11 项指标全部对齐"目前只能追到 5 条指标号）。
这四条每轮都如实登记：不是没排上，是没有环境就验不了对错，硬做只会得到一版"看起来支持、实则没测过"的东西。

### 顺带：几处文档数字已漂

- ✅ `CLAUDE.md` §0 的规模数字（945/92/86 → **946/93/89**）与 §1.12 的"全无 lockfile"（`requirements.lock` 已在用）已改对。
- ☐ `docs/TECH_DEBT.md` **仍标着 X-4～X-9 六条未修**（WS 越权、押金退费非原子、并发退款/结算/缴费超收、网关 pending 不占额度），
  但 `test_ws_auth_boundary.py` 与 `test_billing_money_concurrency.py` 已经连真 PG 复现口径一起钉住了；
  同文件 P1-27 写的 77 处结构漂移，实际已降到 **39（SQLite）/ 41（PG）**。读那份文件的人会以为还有六个未修的金额并发缺陷。
- ☐ `docs/下一步开发计划.md` 整份停留在"50 项测试"的年代，M1/M2 早已完成——建议标注作废而不是继续维护。
