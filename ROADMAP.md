...
- ✅ **分母已扩到「归属隔一跳」（2026-08-30）**：`test_stage15_horizontal.py` 的判据
  从「对象自己带 `org_id`」扩为「自带 `org_id` **或** 外键隔一跳指到带
  `org_id`/`patient_id` 的表」，抽成 `_owning_models()`。
  **关键的一条是排除指向 `users` 的外键**：`created_by`/`requested_by`/`sampler_id`
  这类是审计列不是归属列，顺着走会把「谁创建的」误当成「归属谁」，推出
  「只能改本机构人写的知识条目」这种不存在的规则——实测这一条排除把误报从
  23 压到 12，且剩下每一个都有真实归属路径。
  照出并已修 **11 个真洞**（另 1 个 `notifications.py:mark_read` 登记豁免：它按
  `user_id` 判、比机构更严）：危急值**确认接收 / 处置反馈**（同 `ExamReport`，
  上一轮只修了修订报告、漏了这两条，正因闸门看不见）、住院临床文书四个写接口
  （病程/护理/体温单/交接班，病历是法律文书）、里程碑完成/撤销、慢专病团队成员
  改/删（body 里含五个**权限位**，属权限提升）、随访质控结论、服务包扣减/解绑、
  路径实例调整。回归 `test_cross_org_write_guards.py` 第二批 14 条，
  闸门与行为网**各做过一次变异验证**（拿掉守卫分别转红）。
  同轮把判据的一处假阳性修掉：守卫抽进模块内 `_assert_*` helper 后，
  按字符串认守卫的判据看不见它——现在跟一层本地 helper。
- ☐ **闸门还剩两个结构性盲区（本轮量到，未做）**——记在这里免得又变成「谁也没认领」：
  - **盲区二：对象经 helper 取得**。判据只认函数体里**字面出现** `db.get(M,`，
    经 `_xxx_or_404()` 取件的一概看不见。实测 **50 个** `/{id}` 型写接口落在这里
    （`fund.py` 基金预付/清算/分配、`outpatient_docs` 知情同意签署/拒签、
    `credentials` 凭据作废、`pathology` 标本流转、`spd/tasks` 六个任务动作…）。
    本轮修 `clinical_docs` 那四条时顺手证实了它是真盲区：四个写接口全无守卫，
    闸门只报出唯一那个恰好还有一行字面 `db.get` 的。
  - **盲区三：归属走 body 而非路径参数**。判据只扫 `/{id}` 型，
    `create_handover`（归属在 `body.ward_id`）这类一个都不覆盖。本轮就近修了它，
    但没有量整体面。
  - 另有一次**独立的分母扩大**待做：direct 那一族从「自带 `org_id`」再放宽到
    「自带 `patient_id`」，实测再多约 30 个端点（maternal / insurance /
    vaccination / tcm / emergency / spd/care 等）。刻意不与「隔一跳」混在一批，
    否则分不清哪个洞是被哪次扩大照出来的。
