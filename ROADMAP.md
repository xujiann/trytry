...
- ✅ **P1-38 关账：两套转诊措辞不统一，但并排进同一张表。**
  ...
  **居民看到的是英文状态码**。现在一行一个状态、两个读者两列，想漏也漏不掉。
  ...
  ——那在结构上毫无破绽。
- 🔴 **登记 P1-55：整套用例只在收集顺序下才绿——打乱顺序实测 94 个失败、横跨 51 个文件。**
  根因是共享的 `test_run.db` + 模块级 `reset_database()`：238 个测试文件里 168 个在自己开头
  **把全库 drop 掉重建**，另外 70 个一次都不调、靠继承上一个模块留下的数据跑。
  按字母序收集时这条链恰好接得上，换个顺序就断，失败形态正是「别人的数据」：
  409 冲突、403 越权、`assert 0 == 1`、空列表 IndexError。
  **做了对照实验**（这轮新增的两个测试文件排除后重跑，仍是 94 个失败），
  所以不是本轮引入的，是存量；`pytest-randomly` 只是碰巧装在本容器里、不在 `requirements-dev.txt`，
  CI 与 `make verify` 都走固定顺序，所以这笔账一直在而没人看见。
  真正的代价不是「将来想并行跑」：**用例之间互相喂数据，意味着一条用例可能靠另一个模块建的行才通过**，
  它自己测的东西是不是真的对，无从判断。修法要逐模块确认它依赖的到底是启动种子还是别人的残留，
  不适合夹带在别的改动里，单独立项。
