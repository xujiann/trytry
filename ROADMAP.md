...
- ◐ 统计簇 `analytics/metrics/reports/performance` 合并口径 → **[ADR-0007](docs/adr/0007-统计簇口径合并.md)（Accepted）**。**第一步已交付**：`docs/统计口径对照表.md` —— 逐条比对四个模块的实际代码，摸出 5 处口径分歧 + 1 处更严重的问题：
  ...
  - ☐ **新增待裁定**：共享诊断中心的诊断工作量目前无人计分（见第 2 条）。
  - ☐ 指标名"远程诊断服务量"名不副实（实为申请方平台使用次数）；指标名在可编辑的 `performance_indicators` 表里、种子只增不改，改名会与现场改过名的库不一致，另案。
  ...
  - ☐ 仍**不合并**"月度解析器接受 `YYYY`"这一步：会把 5 个端点当前的 422 变成 200，属对未受影响接口的行为变更，要改走 ADR。已加用例 `test_两个解析器分工不同_月度那个不认年度` 防"顺手统一"。
