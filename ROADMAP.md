...
- ⚠️ **需复核（治理，非技术）**：本文件「统计簇口径」那批里，共享诊断中心计分先记着
  「属新增计分口径、会改基金分配，**需卫健口径支持**」，紧接着的下一条又记着
  「✅ 已修（2026-08-22）……**这会改分数**并影响此后的基金分配」。
  两句不能同时成立：要么口径已获批（那前一句该销案），要么它是在没有裁定的情况下
  改了**决定各机构分多少钱**的算法（那该回退到裁定之后）。请所有者确认属哪一种。
