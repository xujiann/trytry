...
- ✅ **ADR-0009 第八批（2026-09-06）：pages-mgmt.js 四页 20 处外壳换 `panel()`，并揪出一个真缺陷**。
  住院文书 / 手术麻醉 / 流程引擎 / 运营分析四页迁完，36 个夹具页全量重跑逐字符一致。
  第五页 `renderAccounting` **故意没迁**：备夹具时发现它的合并报表面板嵌在凭证表格的行回调里，
  一条凭证渲染一份（夹具两条 → 出现两次，且嵌在 `<tr>` 内），修它会改字节、与"迁移是 no-op"冲突，
  于是登记 TECH_DEBT **P1-41**、留着夹具给修复时做对比。结论记进 ADR：**为迁移造的夹具本身
  就是读页面的一种方式**，比通读模板字符串更容易看出结构性错误。
  剩余手写外壳 192 → **172**（core 44 / clinical 56 / mgmt 27 / public 25 / spd 20）。
