...
- ◐ **分页治理（P2-8）新建闸门 + 首轮清 42 个，欠账 214 → 172。**
  ...
  首轮只做**纯输出上限**那一类：`limit` 入参默认值取原来写死的硬上限，于是不传参的
  老调用方拿到的行一字不差，只多一个 `X-Total-Count` 响应头（响应头不属于 body 字节）。
  刻意没动两类并各自写明理由：**扫描上限**（`.limit()` 限扫描范围、输出还要再筛或
  重排，改 paginate 会翻错页、总数也会报成扫描池大小，两处已就地注明）；
