...
| P1-40 | **全平台孤儿端点**（945 个端点里前端找不到调用点，严格判据；只减不增）。轨迹：241 → 232（扣掉机器对机器豁免）→ 220 → 199（K3 四模块）→ 188（spd 配置域）→ 176（spd 患者生命周期）→ **164**（居民端 H5）。已清零并登记进 `FULLY_COVERED` 的模块：`spd.config.scales` / `routers.portal` 等 43 个。剩余缺口最大：`spd.assess 12`、`spd.tasks 10`、`spd.followup 9`、`spd.portal 9`、`routers.billing 7`、`spd.config.devices 7` | `server/tests/test_frontend_endpoint_coverage.py` |
