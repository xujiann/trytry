...
| P1-40 | **全平台孤儿端点**（948 个端点里前端找不到调用点，严格判据；只减不增）。轨迹：241 → 232 → 220 → 199 → 188 → 176 → 164 → 154 → 145 → 133 → 124 → 104 → 82 → 62 → 43 → **29**（医嘱执行 + 传染病报告卡 + 抢救转归 + 缺药结案 + DRG 事前事中 + 同意撤回 + 慢病风险 + 老年统计）。已清零并登记进 `FULLY_COVERED` 的模块：73 / 92；剩余 19 个模块有缺口（最大 2 个）。 | `server/tests/test_frontend_endpoint_coverage.py` |
| P1-52 | **前端渲染字段名与后端契约脱节**（写成 `i.result` 而契约是 `result_value`，界面上一列 undefined，而孤儿棘轮/转义棘轮/契约棘轮全绿）。已建闸门按 `response_model` 的 `model_fields` 现算比对，覆盖 9 条 (页面, 变量, 契约) 三元组；受限于近半数端点仍返回裸 dict（P1-38），覆盖面只能随契约治理一起扩。 | `server/tests/test_frontend_field_names.py` |
