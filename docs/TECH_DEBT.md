...
| P2-31 | **渲染器「画表单 → await 取数 → 挂监听」的窗口**（2026-08-31 CI 实锤）：窗口里提交表单没人接管，浏览器走原生 GET——数据泄进 URL、整页重载、POST 不发出（现场 `/?enrollment_id=1&template_id=1#spdpath`，两次 CI 失败同形）。已修两层：renderSpdPath 根修（监听先于 await，静态钉 `tests/test_native_submit_guard.py`）+ shared.js document 层兜底 preventDefault（三端全覆盖，把「导航+丢数据」降级为「这一下没生效」）。**剩余欠账**：其余页面渲染器同样的窗口普遍存在（pages-*.js / m.js / doctor.js 里 innerHTML 与监听挂载之间隔 await 的写法），兜底之下不再丢数据但仍是「点了没反应」——逐页把挂监听挪到 await 之前属独立清理任务，动到哪页顺手修哪页（童子军法则第 6 条） |
