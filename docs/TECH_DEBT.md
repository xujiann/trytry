...
| P1-43 | **横向越权棘轮的判据比缺陷窄两层**（修 P1-42 时发现，是形状不是个案）：`test_stage15_horizontal.py::_byid_org_write_endpoints` ①只看**路径带 `{}`** 的端点，②只认 `db.get(Model, ...)`。而 P1-42 那个洞是"**id 从 body 收**、用 `db.query(...).filter(...id.in_(...))` 批量取"——两条判据都绕过去了，于是棘轮一路全绿。按放宽后的形状扫 `app/routers` + `app/spd/routers`，**16 处**命中（appointments:batch_create_slots、clinical_docs:create_handover、contracts:sign、fund:distribute、org_groups:remove_member、pharmacy:transfer_stock/upsert_stock、quality:upsert_medical_record、spd/assess:redeem/signin、spd/followup:auto_match_plans/plan_qc、spd/population:distribute_candidates(已修)/handle_service_apply、spd/tasks:adjust_path_instance/batch_tasks）。**这 16 处是待逐条判定的候选，不是 16 个已证实的越权**——多数可能是"新建行"而非"改别家的行"。要做的是：逐条判定 → 该修的修、按设计跨机构的写进豁免清单并写理由 → 再把放宽后的判据接进棘轮（不先判定就放宽会让 CI 挂在一堆未核实的名字上）。ADR-0009 第二批记过同一条教训：**判据比缺陷窄，等于给缺陷留了个看不见的后门——守卫是绿的** | `server/tests/test_stage15_horizontal.py:484` |
