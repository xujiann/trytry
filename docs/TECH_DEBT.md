...
| P1-40 | **全平台孤儿端点**（945 个端点里前端找不到调用点，严格判据；只减不增）。轨迹：241 → 232（扣掉机器对机器豁免）→ 220 → 199（K3 四模块）→ 188（spd 配置域）→ 176（spd 患者生命周期）→ 164（居民端 H5）→ 154（spd 任务工作流）→ 145（居民端慢专病三视图）→ 133（spd 考核与积分）→ 124（spd 随访中心）→ **104**（住院押金/收费项 + spd 设备与数据源 + 病种目录）。已清零并登记进 `FULLY_COVERED` 的模块：49 个。剩余缺口最大：`routers.education 6`、`routers.users 6`、`routers.appointments 5`、`routers.exams 5` | `server/tests/test_frontend_endpoint_coverage.py` |
