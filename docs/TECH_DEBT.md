...
| P1-40 | **全平台孤儿端点**（948 个端点里前端找不到调用点，严格判据；只减不增）。轨迹：241 → 232（扣掉机器对机器豁免）→ 220 → 199（K3 四模块）→ 188（spd 配置域）→ 176（spd 患者生命周期）→ 164（居民端 H5）→ 154（spd 任务工作流）→ 145（居民端慢专病三视图）→ 133（spd 考核与积分）→ 124（spd 随访中心）→ 104（住院押金/收费项 + spd 设备与数据源 + 病种目录）→ 82（继续教育与健康宣教 + 用户与审计 + 预约 + 检查检验）→ **62**（会诊专家与计费 + 医疗质量 + spd 路径节点/服务包/转诊）。已清零并登记进 `FULLY_COVERED` 的模块：58 / 92。剩余 34 个模块有缺口，分布已摊平（最大 3 个）；逐模块数目见闸门自证输出。 | `server/tests/test_frontend_endpoint_coverage.py` |
