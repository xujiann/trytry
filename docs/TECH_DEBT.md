...
| **P1-57**（新发现，9 处已修） | **按路径 id 经 helper 取实体的写接口，两道横向越权闸门都看不见**。`test_stage15_horizontal.py` 的按 id 写闸门判的是「handler 源码里出现 `db.get(带 org_id 的模型,`」且「出现任一守卫名就算过」——取数写进 `_xxx_or_404` 之类的 helper 就不在它眼里，而 P1-56 的新判据只管请求体里的 id。P1-56 修复过程中**实测确认并已修 9 处**：`clinical_docs` 三个写接口（经 `_admission_or_404`，乙院能往甲院住院患者病历里写病程/护理/体征，201）、`spd/tasks` 六个单条任务写接口（经 `_load_task`，乙院单条认领/催办/升级甲院的任务 200——而批量接口刚被 P1-56 改成跳过别家的任务，**单条不修，批量那道门一绕就过**；校验已收进 `_load_task` 本身，因为它的调用方全是写接口）。按「写接口 + 路径参数 + 调了本文件里会 `db.get` 的 `_` 开头 helper + handler 内无守卫名」**粗筛出 38 处候选、未逐条研判**，其中含已知的假阳性（远程会诊按设计跨机构、已在 `BYID_CROSS_ORG_OK`）；抽查 5 个 helper 内部都没有守卫（`fund._pool`、`pathology._specimen`、`projects._project`、`consultations._get`、`spd/tasks._load_task`）。修法与 P1-56 同：按实体自己的机构判；闸门侧要把 helper 的返回值当作取数点跟进去 | `tests/test_stage15_horizontal.py`；候选见 P1-56 提交说明 |
