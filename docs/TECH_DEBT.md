...
| P1-58 | **跨机构单据缺"处理方机构"列，归属无从判定**（P1-57 研判时发现，按豁免处理）。`emergency_cases` 只有 `dest_org_id`（接收医院），没记 120 调度方/车组的机构——院前推进、体征回传、绿道节点任何机构的经办/医师都能写；`tcm_dispense_orders` 只有下单机构，没记是哪家共享中药房在调配煎煮；`pathology_specimens`、`spd_service_packages` 干脆没有机构列；`spd_referral_rules` 是全县配置、没有归属机构。补列属数据模型变更（迁移 + 存量回填口径），需先定"谁是处理方"的业务口径 | `app/models/emergency.py` / `app/models/pharmacy.py` |
