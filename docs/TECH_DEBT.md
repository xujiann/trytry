...
| P2-8 | paginate 仅 39/96 文件；197 处硬编码 `.limit(N)`——其中 **162 个 GET 列表端点会随数据量静默截断**（无 `X-Total-Count`、无 `offset`，调用方连「少没少」都不知道）。◐ 2026-09-06 起逐块切：已量化基线 170，切完 `billing.py` 8 个降到 **162**，并加棘轮 `tests/test_list_pagination_ratchet.py` 只减不增 |
