...
| P2-26 | 33 处状态标签在手工重复 `spdTag()` 已经做对的事 | ☐ **待办**：`pages-spd.js:41` 的 `spdTag(map, key)`（与 `m/m.js:919` 的 `spdTagOf` 逐字相同）正是"把转义收进组件"的成品——`map[key] \|\| [key \|\| "—", ""]` + `esc(text)`。P2-24 修的那 33 处全是在手抄它。收敛过去正是 ADR-0009 的论点，但**不是纯 no-op**（空状态码下 `spdTag` 显示 `—`、手写版显示空），要按页取证，故随 ADR-0009 逐页迁移一起做，别单独批量替换 |
