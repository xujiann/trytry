...
| P1-41 | **会计页的合并报表面板嵌在凭证表格的行模板里，一条凭证渲染一份**（ADR-0009 第八批备夹具时发现）：`renderAccounting` 里 `<div class="panel"><h3>合并报表…` 整段落在凭证 `table()` 的行回调返回值里，也就是每条凭证都会重复输出一整份合并报表（且嵌在 `<tr>` 内，浏览器会把它甩出表格）。实测：夹具给两条凭证 → 页面上"合并报表"出现 **2 次**；生产上 30 条凭证就是 30 份。成因是模板字符串的反引号收错了位置，不是逻辑写错。**修它会改字节**，所以第八批没顺手改（迁移那一步要求 no-op），也因此 `renderAccounting` 这一页整页留到修完再迁。修法：把合并报表那段从行回调里提出来，放回 `innerHTML` 顶层的面板序列中，用 `node scripts/render_diff.js accounting` 对比修前修后（夹具 `accounting` 已就位） | `server/app/static/pages-mgmt.js` 的 `renderAccounting`；夹具 `scripts/fixtures/render_fixtures.json` 的 `accounting` |
