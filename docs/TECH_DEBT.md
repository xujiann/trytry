...
| P1-52 | **前端渲染字段名与后端契约脱节**（写成 `i.result` 而契约是 `result_value`，界面上一列 undefined，而孤儿棘轮/转义棘轮/契约棘轮全绿）。已建闸门按 `response_model` 的 `model_fields` 现算比对，覆盖 9 条 (页面, 变量, 契约) 三元组；受限于近半数端点仍返回裸 dict（**P2-5**，不是 P1-38——后者是居民端转诊措辞），覆盖面只能随契约治理一起扩。 | `server/tests/test_frontend_field_names.py` |
