...
| **P1-56**（新发现，1 处已实测放行） | **归属校验校的是一个自报字段**。P1-39 的静态闸门（`test_body_org_write_guard.py`）判「写接口调没调 `assert_org_writable`」——只看函数名出现在源码里，**不看它校验的值可不可信**。于是凡是「请求体里自报一个机构 id 拿去校验、同时又引用了一个**自带机构归属**的实体」的写接口，都可能靠谎报那个机构 id 绕过：P1-51 的派驻就是这么被绕过的。按形状扫出 7 处候选（`db.get(自带 org_id 的模型, body.xxx)` + `assert_org_writable(body.<org>)`），1 处函数里已有比对，**6 处未比对**：`vaccination.vaccinate`（`VaccineBatch`）、`vaccine_supply.report_aefi`（`VaccinationRecord`）、`materials.create_purchase`（`Department`）、spd 的 `teams.create_village_doctor`（`User`）、`population.distribute_candidates`（`SpdTeam`）、`tasks.create_task`（`SpdEnrollment`）。**第一处已实测放行**：乙院医生填 `org_id=乙院`、`batch_id=甲院的疫苗批次`，接种 201，**甲院批次已用量 0→1**——别家的库存被扣，接种记录的批号指向乙院从未持有过的批次，冷链追溯就此断在一条假记录上。其余 5 处未逐一实测（扫描是文本启发式，可能有以别的写法比对过的）。修法都是一行：取出实体后比对它的机构，或直接按实体的机构判归属（P1-51 的做法）。闸门侧要补的是「校验值须来自服务端事实」这条判据，单列一项，不与逐处修复混在一个提交里 | `app/routers/vaccination.py` 等 6 处；`tests/test_body_org_write_guard.py` |
