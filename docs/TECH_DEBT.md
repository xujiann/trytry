...
| P1-30 | **并发防复发闸门覆盖面仅 27.5%**：229 个 `db.add` 写入点里，"唯一表"判据只覆盖 63 个，其余 166 个规则完全不看（未覆盖最多：`critical_actions` 5、`exchange_logs`/`followup_tasks`/`satisfaction_surveys`/`spd_screenings` 各 3）。补强前更低：87 文件 / 211 写入点 / 覆盖 42 个（19.9%），且 `app/spd/routers/config/` 整个子包（8 文件 18 写入点）从未被扫过。**2026-09-03 进行中——缺口已从"看不到"变成"审过"**：未覆盖的 136 张表逐张审计（分类 → 怀疑者复审 → 业务/代码/数据三视角试图推翻），112 张判多行合法/由别处守住（`AUDITED_MULTI_ROW_TABLES`，条条有理由）、5 张待决（`AUDITED_UNDECIDED_TABLES`：`child_records`/`emergency_resources`/`followup_tasks`/`report_templates`/`spd_interventions`，键在表上表达不出来或要产品决定）、**20 张判业务上确实唯一**（`disease_enrollments` 在管唯一、`delivery_records` 每册一条、`spd_consults`/`spd_service_applies`/`dual_channel_apps`/`special_disease_apps` 待处理唯一、`fund_distributions`/`spd_program_versions`/`spd_qc_samples`/`spd_package_bindings`/`spd_lifecycle_events`/`inpatient_orders`/`reconciliation_batches`，以及唯一性长在父行上的 `charge_price_changes`/`spd_point_records`/`spd_tasks`/`spd_referral_steps`/`workflow_transitions`/`asset_movements`/`spd_call_tasks`），去向是唯一索引下沉 + `insert_or_conflict` 或父行条件 UPDATE，落地中。新增棘轮：既未覆盖也未审计的写入点 26 → 只减、待决 8 → 只减；扫描器补认 `for x in xs: db.add(x)` 形状，未识别 1 → 0 | 自证用例 `tests/test_stage14_concurrency.py::test_防复发闸门自证覆盖面`（每次运行打印分母，缺口显式化）；清单防腐 `test_已审计清单不得腐烂` |
