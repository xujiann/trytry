...
| P1-40 | **全平台孤儿端点**（948 个端点里前端找不到调用点，严格判据；只减不增）。轨迹：241 → 232 → 220 → 199 → 188 → 176 → 164 → 154 → 145 → 133 → 124 → 104 → 82 → 62 → **43**（药品召回链 + 审方规则 + 消供申领 + 一码通 + 死因报告卡 + 物资调拨）。已清零并登记进 `FULLY_COVERED` 的模块：65 / 92；剩余 27 个模块有缺口，分布摊平（最大 2 个），逐模块数目见闸门自证输出。 | `server/tests/test_frontend_endpoint_coverage.py` |
