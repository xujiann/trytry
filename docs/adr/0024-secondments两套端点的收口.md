...
> （P1-58 开工时那条棘轮迁到了 `tests/test_date_query_params.py`，判据补全后为 27 条：
> 剔除 2 条误报、数进 4 条原先没数到的——见 TECH_DEBT P1-58。）
...
>
> 📌 **2026-09-18 P1-59 取证结果（并更正本段初稿的一处措辞）**：逐条实测后，
> 真正可越权的是**两条**而非三条——`second_employee` 与 `create_staff_contract`
> 被**乙院 operator** 对甲院医师调用时各 201 落库，均已补上归属校验并带越权反例用例。
> `create_payroll` 的角色门是 `require_roles("director")`，而 `director` 属
> `visibility.GLOBAL_ROLES`（**跨机构是设计如此**，统筹医共体人事），
> 能调它的角色本就全域，故它不是可越权入口；本批给它补的是纵深防御。
> 本段初稿说"A 院的 director 可以…"用错了角色，特此更正。
>
