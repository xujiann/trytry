...
- 类型约定（照抄现状，别自创）：
  ...
  - **必填文本**：请求字段写 `Field(min_length=1, max_length=N, pattern=NON_BLANK)`（`app/texttypes.py`）——光有 `min_length` 挡不住一串空格，没有默认值却不写 `min_length` 的连空串都收；出参继承到它要覆盖回不带它的声明（`tests/test_blank_required_text.py` 两侧都盯着，P1-109 / P1-110）。
...
- **孤儿端点棘轮**（`tests/test_orphan_endpoints.py`）：新端点要么同一批带界面，要么进 `EXEMPT_*` 写明**为什么不需要界面**；
  ...
  它按路径算：清单有页面调，路径就算接上了——「新建」「删除」这类写动词有没有入口，由动词级棘轮（`tests/test_orphan_endpoint_verbs.py`，P2-93）另外对一遍，名单同样只减不增。
